...
import { channelColor, sourceColor } from "@/lib/presentation";
import { STATUS_BUCKET_LABELS, STATUS_BUCKETS } from "@/lib/types";
import { STATUS_BUCKET_COLOR } from "@/lib/presentation";

export const metadata = { title: "Calendar · AFC Command Center" };
...
export default async function CalendarPage({
  ...
  return (
    <AppShell
      snapshot={snapshot}
      title="Calendar"
      subtitle={`${items.length} scheduled or dated items`}
      toolbar={
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <ChipGroup
            ...
            label="Source"
            options={[
              { value: "content", label: "Content", color: sourceColor("content") },
              { value: "shorts", label: "Shorts", color: sourceColor("shorts") },
              { value: "clips", label: "Clips", color: sourceColor("clips") },
            ]}
          />
          {channels.length > 0 && (
            <ChipGroup
